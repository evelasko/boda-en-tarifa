...
async function deleteUsersWithPrefix(prefix) {
    const auth = getAuth();
    const uids = [];
    let pageToken;
    do {
    ...
    for (let i = 0; i < uids.length; i += chunkSize) {
        ...
        if (result.failureCount > 0) {
            for (const err of result.errors) {
                ...
                console.warn(`Failed to delete auth user ${uid}: ${err.error.message}`);
            }
        }
    }
    return deleted;
}
async function deleteDocsByIdPrefix(collectionName, prefix) {
...
async function deleteExplicitDocs(collectionName, ids) {
    if (ids.length === 0)
        return 0;
    const db = getFirestore();
    let deleted = 0;
...
async function main() {
    ...
    const sentNotificationsDeleted = await deleteExplicitDocs("sent_notifications", ["welcome_party_reminder"]);
    ...
    console.log(`- rsvp_responses deleted: ${rsvpDeleted}${includeRsvp ? "" : " (skipped)"}`);
}
main().catch((error) => {
...
//# sourceMappingURL=clear-emulator.js.map