...
const singleGuestEmail = guestFlagIndex !== -1 ? args[guestFlagIndex + 1] : null;
...
// ---------------------------------------------------------------------------
const keyPath = process.env.GOOGLE_APPLICATION_CREDENTIALS || args.find((a) => a.endsWith(".json"));
if (!keyPath) {
...
async function main() {
    ...
    for (const doc of snapshot.docs) {
        ...
        try {
            ...
            console.log(`${csvEscape(fullName)},${csvEscape(email)},${csvEscape(deepLinkUrl)}`);
        }
        catch (err) {
            console.error(`Failed to generate token for ${fullName} (${uid}):`, err instanceof Error ? err.message : err);
        }
    }
}
...
//# sourceMappingURL=generate-magic-links.js.map