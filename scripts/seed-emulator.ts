...
type CustomClaims = Record<string, boolean>;
...
function parseIncludeMigrationCase(args: string[]): boolean {
...
function nowServerTimestamps(): Pick<DocumentData, "createdAt" | "updatedAt"> {
  return {
...
function buildGuests(includeMigrationCase: boolean): Record<string, DocumentData> {
  const common = nowServerTimestamps();

  const baseGuests: Record<string, DocumentData> = {
    seed_admin_001: {
      email: "admin.one@example.test",
      fullName: "Admin One",
...
function buildSeating(): Record<string, DocumentData> {
  return {
...
function buildTimeGatedContent(): Record<string, DocumentData> {
  return {
    cocktail_menu: {
    ...
    seating_chart: {
      ...
      content: {
        totalTables: 12,
        floorPlanUrl:
          "https://res.cloudinary.com/demo/image/upload/v1/seed/floor-plan.png",
      },
    },
    banquet_menu: {
...
function buildFeedPosts(): Record<string, DocumentData> {
...
function buildNotices(): Record<string, DocumentData> {
  const authors = [
    {
      uid: "seed_guest_001",
      name: "Ana del Mar",
  ...
  const notices: Record<string, DocumentData> = {};
...
function buildSentNotifications(): Record<string, DocumentData> {
  return {
...
function buildRsvpResponses(): Record<string, DocumentData> {
  const base = {
...
async function upsertCollection(
  ...
  records: Record<string, DocumentData>
): Promise<number> {
...
async function upsertAuthUsers(users: SeedAuthUser[]): Promise<number> {
  const auth = getAuth();
  let count = 0;
...
async function main(): Promise<void> {
  ...
  const guestsSeeded = await upsertCollection(
    "guests",
    buildGuests(includeMigrationCase)
  );
  const seatingSeeded = await upsertCollection("seating", buildSeating());
  const timeGatedSeeded = await upsertCollection(
    "time_gated_content",
    buildTimeGatedContent()
  );
  ...
  const sentNotificationsSeeded = await upsertCollection(
    "sent_notifications",
    buildSentNotifications()
  );
  ...
  console.log(
    `- migration case: ${includeMigrationCase ? "included" : "skipped"}`
  );
}
