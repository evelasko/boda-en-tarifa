...
import {contentUnlockNotifications} from "../config/notifications.js";

/**
 * Config-driven content unlock notification function.
 *
 ...
 */
export const sendContentUnlockNotification = onSchedule(
  {
    schedule: "*/15 * 29-31 5 *",
  ...
  async () => {
    ...
    const dueNotifications = contentUnlockNotifications.filter(
      (n) => n.enabled && new Date(n.sendAt) <= now
    );

    if (dueNotifications.length === 0) {
      logger.info("No content unlock notifications due at this time.");
      return;
    }
    ...
    logger.info("sendContentUnlockNotification completed.");
  }
);
